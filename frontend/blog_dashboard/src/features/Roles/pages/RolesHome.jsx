...
export function RolesHome() {
    ...
    const { data, isLoading, refetch } = useQuery([filters], fetchRoles)
    useEffect(() => {
        refetch()
    }, [filters])
    ...
    return (
        <Box width="100%">
            <Box>
                ...
                <Box>
                    {isLoading && <Loading />}
                    {data && data?.roleList?.length > 0 && !isLoading && (
                        <>
                            <Box mb="16px">
                                <Data data={{ dataHeader, dataBody: data.roleList }} />
                            </Box>
            ...
            <ModalDelete fetchApiData={rolesApi} />
        </Box>
    )
}
